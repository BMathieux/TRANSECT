classdef TRANSECT
    %TRANSECT object from baseline coordinates on DEM
    %
    % Syntax
    %
    ...
    %
    %     Indexing convention:
    %         i1 = side of baseline (1 = left, 2 = right)
    %         i2 = baseline node index
    %         i3 = connected path index linking back to baseline node i2
    %
    %     For easier access to paths and arrays, use the helper function:
    %         val = extract(T, i1, i2, i3, 'x');                % from int
    %         val = extract(T, i1, i2, i3, 'z', 'group','conn'); % from conn
    %
    % Input arguments
    %
    ...
    % Methods
    %
    %     calcStats  – compute cross-sectional and slope statistics
    %     resample   – resample transect paths on a given DEM 
    %
    % Properties
    %
    ...
    %
    % Example (using TopoToolbox)
    %
    %     % Load DEM
    %     DEM = GRIDobj(dem_lauch_5m.tif');
    %
    %     % Delineate trunk river (using STREAMobj)
    %     FD  = FLOWobj(DEM,'preprocess','carve');
    %     S   = STREAMobj(FD,'minarea',1e6);
    %     S   = trunk(S);                     % extract trunk stream
    %
    %     % Use trunk coordinates as baseline
    %     x = S.x; 
    %     y = S.y;
    %
    %     % Create transects perpendicular to trunk
    %     T = TRANSECT(DEM,x,y,'w',10,'ite',3);
    %
    %     % Plot DEM and trunk with transects
    %     figure, imageschs(DEM), hold on
    %     plot(S,'b'), hold on, plot(T)
    %     
    %     Note
    %     ----
...
%                             p = all_paths{i}{j};
            ...
            if strcmp(method,'geometric')
                obj = obj.resample(DEMi);
            end

            obj = obj.calcStats('func',@mean);
        end
    end
    ...
    methods
        function obj = calcStats(obj,varargin)
        % stats Compute transect statistics (single-path only)
        %
        % Syntax:
        %   obj = obj.calcStats()
        %   obj = obj.calcStats('func',@median)
        %
        % Options:
        ...
        % init stats struct
        obj.stats.x = cell(n,1);
        obj.stats.y = cell(n,1);
        obj.stats.z = cell(n,1);
        obj.stats.d = cell(n,1);
        obj.stats.Z1 = nan(n,1);
        obj.stats.Z2 = nan(n,1);
        obj.stats.G1 = nan(n,1);
        obj.stats.G2 = nan(n,1);
        obj.stats.W  = nan(n,1);
        obj.stats.H  = nan(n,1);
        
        for i = 1:n
            try
                % always take single middle path on each side
                midL = ceil(numel(obj.int{1}(i).x)/2);
                xL = flip(obj.int{1}(i).x{midL});
                yL = flip(obj.int{1}(i).y{midL});
                zL = flip(obj.int{1}(i).z{midL});
                dL = flip(obj.int{1}(i).d{midL} * -1);

                midR = ceil(numel(obj.int{2}(i).x)/2);
                xR = obj.int{2}(i).x{midR};
                yR = obj.int{2}(i).y{midR};
                zR = obj.int{2}(i).z{midR};
                dR = obj.int{2}(i).d{midR};
            ...
            if any(id1), obj.stats.Z1(i) = max(zsec(id1)); end
            if any(id2), obj.stats.Z2(i) = max(zsec(id2)); end
            if any(id1(:)) && any(id2(:))
                obj.stats.W(i) = abs(min(dsec(id1))) + max(dsec(id2));
                obj.stats.H(i) = mean([obj.stats.Z1(i) obj.stats.Z2(i)],'omitnan') - min(zsec);
            end
            
            % Slope metrics (with custom function)
            if sum(id1)>1
